...
namespace df::bifrost {
	...
	void Asgard::handleClient(int socket, std::stop_token stopToken) {
		...
		while (!stopToken.stop_requested() && running_) {
			...
			if (msg.type == MessageType::RECONNECT) {
				...
				if (playerIdOpt) {
					...
					if (session_.getState() == SessionState::PLAYING ||
						...
						respPayload.state = session_.getSerializedGameStateForSocket(socket);
					}
			...
			if (result.broadcastGameStarted) {
	...
	void Asgard::broadcastGameState() {
		auto sockets = getAllSockets();
		for (int socket : sockets) {
			Message msg = createGameStateMessage(0, session_.getSerializedGameStateForSocket(socket));
			sendToClient(socket, msg);
		}
	}

	void Asgard::broadcastGameStarted() {
		auto sockets = getAllSockets();
		for (int socket : sockets) {
			Message msg;
			msg.type = MessageType::GAME_STARTED;
			msg.seq = 0;
			msg.payload = GameStartedPayload{session_.getSerializedGameStateForSocket(socket)};
			sendToClient(socket, msg);
		}
	}
