...
namespace df::bifrost {
	...
	void Midgard::upgradeSettlement(size_t settlementId, types::SettlementType targetType) {
		if (!connected_) {
			return;
		}

		Message msg;
		msg.type = MessageType::UPGRADE_SETTLEMENT;
		msg.seq = getNextSeq();
		msg.payload = UpgradeSettlementPayload{settlementId, targetType};

		sendMessage(msg);
	}


	void Midgard::buildProductivityBuilding(size_t tileId, types::TileType tileType) {
		if (!connected_) {
			return;
		}

		Message msg;
		msg.type = MessageType::BUILD_PRODUCTIVITY_BUILDING;
		msg.seq = getNextSeq();
		msg.payload = BuildProductivityBuildingPayload{tileId, tileType};
