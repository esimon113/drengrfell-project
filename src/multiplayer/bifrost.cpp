...
namespace df::bifrost {
	...
	namespace detail {
		...
		std::string settlementTypeToString(types::SettlementType type) {
			switch (type) {
			case types::SettlementType::WOOD:
				return "WOOD";
			case types::SettlementType::STONE:
				return "STONE";
			case types::SettlementType::CASTLE:
				return "CASTLE";
			}
			return "WOOD";
		}

		types::SettlementType stringToSettlementType(const std::string& str) {
			if (str == "STONE")
				return types::SettlementType::STONE;
			if (str == "CASTLE")
				return types::SettlementType::CASTLE;
			return types::SettlementType::WOOD;
		}
		...
		nlohmann::json serializePayload(MessageType type, const MessagePayload& payload) {
			...
			case MessageType::UPGRADE_SETTLEMENT: {
				const auto& p = std::get<UpgradeSettlementPayload>(payload);
				j["settlementId"] = p.settlementId;
				j["targetType"] = settlementTypeToString(p.targetType);
				break;
			}
			case MessageType::BUILD_PRODUCTIVITY_BUILDING: {
				const auto& p = std::get<BuildProductivityBuildingPayload>(payload);
				j["tileId"] = p.tileId;
		...
		MessagePayload deserializePayload(MessageType type, const nlohmann::json& j) {
			...
			case MessageType::UPGRADE_SETTLEMENT: {
				UpgradeSettlementPayload p;
				p.settlementId = j.value("settlementId", size_t{0});
				p.targetType = stringToSettlementType(j.value("targetType", "STONE"));
				return p;
			}
			case MessageType::BUILD_PRODUCTIVITY_BUILDING: {
