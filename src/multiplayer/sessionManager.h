	...
	class SessionManager {
	  public:
		...
		struct MessageResult {
			...
			bool broadcastGameStarted{false};
		...
		/**
		 * Move hero. At most one successful move per turn.
		 */
		[[nodiscard]] std::pair<bool, std::optional<ErrorInfo>> moveHero(int socket, size_t targetTileId);

		/**
		 * Upgrade a settlement (WOOD -> STONE -> CASTLE).
		 */
		[[nodiscard]] std::pair<bool, std::optional<ErrorInfo>> upgradeSettlement(int socket, size_t settlementId, types::SettlementType targetType);

		/**
		 * Place a productivity building on a tile.
		 */
		...
		[[nodiscard]] nlohmann::json getSerializedGameStateForSocket(int socket) const;
