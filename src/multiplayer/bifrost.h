 ...
 * - Per-player State Sync: After each accepted action, each client receives
 *   GameState JSON filtered for that player (hidden information). Do not use
 *   Graph::deserialize; clients regenerate the map from world seed/config.
...
namespace df::bifrost {
	...
	struct UpgradeSettlementPayload {
		size_t settlementId{0};
		types::SettlementType targetType{types::SettlementType::STONE};
	};

	struct BuildProductivityBuildingPayload {
		size_t tileId{0};
